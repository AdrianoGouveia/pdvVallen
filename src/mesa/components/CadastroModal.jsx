...
export function CadastroModal({ onConcluido, onFechar }) {
  ...
  async function handleSubmit(e) {
    ...
    if (nascimento.length < 10) { setErro('Digite a data completa: DD/MM/AAAA'); return }
    const dataNasc = parseDDMMAAAA(nascimento)
    if (!dataNasc || isNaN(dataNasc.getTime())) { setErro('Data inválida. Use DD/MM/AAAA'); return }
    ...
    // Verificar cadastro existente pelo telefone
    ...
    if (existente) {
      const is18Existente = calcularIs18(new Date(existente.data_nascimento))
      const clienteData = { id: existente.id, nome: existente.nome, telefone: existente.telefone, is18: is18Existente }
    ...
    const { data: novo, error } = await supabase
      .from('clientes')
      .insert({ nome, telefone, data_nascimento: isoDate })
      .select('id')
      .single()

    if (error) {
      setErro('Erro ao cadastrar. Verifique sua conexão e tente novamente.')
      console.error('Cadastro erro:', error.message)
      setLoading(false)
      return
    }

    const clienteData = { id: novo.id, nome, telefone, is18 }
    localStorage.setItem('mesa_cliente', JSON.stringify(clienteData))
    setLoading(false)
    onConcluido(clienteData)
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <input value={form.telefone} onChange={e => set('telefone', e.target.value)}
              placeholder="11987654321" type="tel" inputMode="numeric" required
              className="w-full bg-vallen-dark border border-vallen-border rounded-xl px-4 py-3.5 text-base text-vallen-white focus:outline-none focus:border-vallen-green" />
          </div>

          <div className="space-y-1">
            <label className="text-xs text-vallen-muted font-medium">Data de nascimento</label>
            <input value={form.nascimento} onChange={handleNascimento}
              placeholder="DD/MM/AAAA" inputMode="numeric" maxLength={10} required
